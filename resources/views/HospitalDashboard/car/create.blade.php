...
<form  class="setting_form" method="POST" action="{{ url('car') }}" enctype="multipart/form-data">
    @csrf
    <div class="row">
      <div class="col-lg-12">
                  ...
                  <div class="form-group">
                      ...
                      <select id="hospital_id" class="text_field @error('hospital_id') validation @enderror" name="hospital_id">
                          ...
                          @foreach ($hospitals as $key=>$hospital )
                              <option @if(old('hospital_id')== $hospital->id) selected @endif value="{{$hospital->id}}">{{$hospital->name}}</option>
                          @endforeach
                      </select>  
                      @error('hospital_id')
